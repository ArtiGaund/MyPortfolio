...
float snoise(vec3 v){ 
 ...
 void main()
 {
    ...
    //tilt from us
    float tilt = -0.8 * uv.y;
    //incline will move the plane up and down
    float incline = uv.x * 0.1;

    float offset = incline * mix(-0.25, 0.25, uv.y);
    //applying noise
    float noise = snoise(vec3(noiseCoord.x + u_time*3.0, noiseCoord.y, u_time * 10.0)); //uv for 1st 2 coordinates and time for rest
    //taking only positive noise
    noise = max( 0. , noise); 
    //updating the z position
    vec3 pos = vec3(position.x, position.y, position.z + noise * 0.3 + tilt + incline + offset);
    //setting background with this color
     vColor = uColor[4];

     //applying lots of noise function and creating new color
    for(int i=0;i<4;i++){
        // if written like this noise is calculating same value and updating same spot with different color
